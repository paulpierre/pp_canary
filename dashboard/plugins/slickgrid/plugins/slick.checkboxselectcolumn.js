(function ($) {
  ...
  function CheckboxSelectColumn(options) {
    ...
    function createUID() {
      return '_row_checkbox';
      //return Math.round(10000000 * Math.random());
    }

    function checkboxSelectionFormatter(row, cell, value, columnDef, dataContext) {
      var UID = createUID() +'_'+ row;

      if (dataContext) {
        return _selectedRowsLookup[row]
            ? "<input id='selector" + UID + "' type='checkbox' class='slick-cell-checkboxsel' checked><label for='selector" + UID + "'></label>"
            : "<input id='selector" + UID + "' type='checkbox' class='slick-cell-checkboxsel'><label for='selector" + UID + "'></label>";
      }
      return null;
    }
